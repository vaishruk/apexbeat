...
(function () {
  ...
  const refSection = (M.references && M.references.length)
    ? '<section id="references"><h2><span class="sec-num">§</span> References</h2>' +
      '<div class="callout" style="border-color:var(--line);background:var(--surface-2)"><b style="color:var(--teal)">Using these references</b>' +
      '<p style="margin:6px 0 0;color:var(--muted);font-size:13.5px">Guideline titles are listed for orientation; always confirm specifics against current guidelines and local protocols before clinical use.</p></div>' +
  ...
  root.innerHTML =
    ...
    "</div>" +
    '<div class="review-status"><span class="review-pill"><i></i> Author: <span data-creator>Vaishnavi Sabesan, MD</span></span></div>' +
    "</div></header>" +
